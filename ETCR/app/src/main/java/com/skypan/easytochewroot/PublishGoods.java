...
*/
        ...
        Fabu.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String info=Nr.getText().toString();
                ByteArrayOutputStream stream = new ByteArrayOutputStream();
                bitmap.compress(Bitmap.CompressFormat.PNG, 100, stream);
                byte[] bytes = stream.toByteArray();// 转为byte数组
                //String picturechar="mmp";
                String picturechar=Base64.encodeToString(bytes,Base64.DEFAULT);
                ...
                new Thread(()->{
                    ...
                    FormBody formBody = new FormBody.Builder()
                            .add("new_name", title)
                            .add("new_user_id",post_userid)
                            .add("new_type", kind)
                            .add("new_time", time)
                            .add("new_money", price)
                            .add("new_phone", contact)
                            .add("new_detail", info)
                            .add("new_picture", picturechar)
                            .build();


                    Request request = new Request.Builder()
                            .url("http://121.43.181.253:8080/merch/publish")
                            .post(formBody)
                            .build();
                    try (Response response = okHttpClient.newCall(request).execute()) {
                        ...
                        if (Boolean.parseBoolean(response.body().string()))
                        {
                            Toast.makeText(getApplicationContext(), "添加成功", Toast.LENGTH_SHORT).show();
                             Intent intent=new Intent(PublishGoods.this,Myaty.class);
